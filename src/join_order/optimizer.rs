...
use datafusion::logical_expr::{
    build_join_schema, Aggregate, BinaryExpr, Expr, Filter, Join, Limit, LogicalPlan, Projection,
...
use crate::join_order::query_graph::{leaf_expr, Edge, QueryGraph};
...
fn join_tree_to_df(
    ...
    df_plan: &LogicalPlan,
    graph: &QueryGraph,
    filter_preds: &Vec<Expr>,
) -> LogicalPlan {
    if join_tree.size == 1 {
        assert!(
            join_tree.right.is_none(),
            "tree size is 1, and right is none"
        );
        ...
        let to_apply: Vec<Expr> = filter_preds
            .iter()
            .filter_map(|f| {
        ...
        let scan_node = get_logical_scan_node(df_plan, &full_name).unwrap();

        let scan_node = LogicalPlan::TableScan(
            TableScan::try_new(
                scan_node.table_name.clone(),
                scan_node.source.clone(),
                scan_node.projection.clone(),
                scan_node.filters.clone(),
                scan_node.fetch,
            )
            .unwrap(),
        );
        ...
        if &full_name == t_name {
            if !to_apply.is_empty() {
                return LogicalPlan::Filter(
                    Filter::try_new(to_apply_expr.unwrap(), Arc::from(scan_node)).unwrap(),
                );
            }
            return scan_node;
        }
        ...
        if !to_apply.is_empty() {
            ...
            return LogicalPlan::SubqueryAlias(
                SubqueryAlias::try_new(Arc::from(f), t_name).unwrap(),
            );
        }

        return LogicalPlan::SubqueryAlias(
            SubqueryAlias::try_new(Arc::from(scan_node), t_name).unwrap(),
        );
    }
    ...
    let exprs: Vec<(Expr, Expr)> = join_tree
        .edges
        .iter()
        .map(edge_to_expr)
        .map(|e| reorder_expr_tuple(e, &left_tree, &right_tree))
        .collect();
...
fn get_table_from_expr(expr: &Expr) -> Option<String> {
    match expr {
        Expr::Column(c) => Some(c.to_owned().relation.unwrap().to_string()),
        Expr::BinaryExpr(be) => {
            let left = &be.left;
            let right = &be.right;
        ...
        Expr::Like(like) => get_table_from_expr(&like.expr),
        Expr::IsNull(null) => get_table_from_expr(null),
        Expr::Between(between) => get_table_from_expr(&between.expr),
        Expr::IsNotNull(nn) => get_table_from_expr(nn),
        Expr::InList(list) => get_table_from_expr(&list.expr),
        node => panic!("unexpected: {:#?}", node),
    }
}

fn reorder_expr_tuple(
    exprs: (Expr, Expr),
    left_tree: &JoinTree,
    right_tree: &JoinTree,
) -> (Expr, Expr) {
    ...
    let left_table = if let Expr::Column(c) = exprs.0 {
        c.relation.unwrap()
    } else {
        panic!("not column")
    };
    let right_table = if let Expr::Column(c) = exprs.1 {
        c.relation.unwrap()
    } else {
        panic!("not column")
    };
...
fn get_logical_scan_node<'a>(df_plan: &'a LogicalPlan, table_name: &str) -> Option<&'a TableScan> {
    match df_plan {
...
fn remove_filter_node(plan: &LogicalPlan) -> LogicalPlan {
    let new_plan = plan
        ...
        .transform_down(|node| {
            ...
            let is_child_filter = inputs
                .iter()
                .any(|child| matches!(child, LogicalPlan::Filter(_)));
            if is_child_filter {
                ...
                if let LogicalPlan::Filter(f) = inputs.first().unwrap() {
                    ...
                    match node {
                        LogicalPlan::Aggregate(a) => {
                            let new_aggr =
                                Aggregate::try_new(filter_child, a.group_expr, a.aggr_expr);
