...
impl QueryGraph {
    ...
    fn enumerate_csg_rec<'a>(&'a self, set: HashSet<&'a Relation>, exclude: HashSet<&'a Relation>) -> Vec<HashSet<&'a Relation>> {
    ...
    fn enumerate_cmp<'a>(&'a self, set: &HashSet<&'a Relation>) -> Vec<HashSet<&'a Relation>> {
        ...
        let exclude: HashSet<&Relation> = self.nodes_with_eq_smaller_id(min.id).union(&set).copied().collect();
        let n = self.neighbourhood(set.clone()).sub(&exclude);
    ...
    fn neighbourhood_single_relation(&self, relation: &Relation) -> HashSet<&Relation> {
        ...
        for edge in &self.edges {
            if edge.node1 == relation.name || edge.node2 == relation.name {
                let r1 = self.nodes.iter().find(|rel| rel.name == edge.node1).expect("cant find relation in nodes");
                let r2 = self.nodes.iter().find(|rel| rel.name == edge.node2).expect("cant find relation in nodes");
                res.insert(r1);
                res.insert(r2);
            }
        }
...
fn subsets_rec<'a>(set: &Vec<&'a Relation>, res: &mut Vec<Vec<&'a Relation>>, curr: &mut Vec<&'a Relation>, index: usize) {
    res.push(curr.clone());
...
fn assign_ordering(graph: &Graph) -> Vec<Relation> {
    ...
    while !q.is_empty() {
        ...
        new_nodes.push(Relation {name: curr.to_string(), id: i});
        i += 1;
